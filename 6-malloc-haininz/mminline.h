...
static inline long *block_end_tag(block_t *b) {
    ...
    // According to the handout: The last element of the payload array should
    // contain the block’s end tag.
    return (long *)((char *)b + (b->size & -2) - sizeof(long));
}
...
static inline int block_prev_allocated(block_t *b) {
    // TODO: Implement this function
    long *prevEndTag = (long *)((char *)b - sizeof(long));
    return (*prevEndTag) & 1;
}
...
static inline block_t *payload_to_block(void *payload) {
    // TODO: Implement this function
    return (block_t *)((char *)payload - sizeof(long));
}
...
static inline void insert_free_block(block_t *fb) {
    ...
    if (flist_first == NULL) {
        ...
        block_set_blink(fb, block_blink(flist_first));

        block_set_flink(block_blink(flist_first), fb);
