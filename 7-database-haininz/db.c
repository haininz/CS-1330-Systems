...
node_t *node_constructor(char *arg_key, char *arg_value, node_t *arg_left,
    ...
    pthread_rwlock_init(&new_node->lock,
                        NULL);  // Initialize the read-write lock

    return new_node;
}

void node_destructor(node_t *node) {
    pthread_rwlock_destroy(&node->lock);  // Destroy the read-write lock
...
node_t *search(char *key, node_t *parent, node_t **parentpp, int write) {
    /*
     * TODO:
    ...
    if (next == NULL) {
        ...
        if (strcmp(key, next->key) == 0) {
            ...
            return search(key, next, parentpp, write);
        }
    }
